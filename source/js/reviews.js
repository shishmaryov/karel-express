window.addEventListener('DOMContentLoaded', () => {
    ...
    function submitHandler2(thisForm) {
        ...
        xhr.open('POST', '../mail.php', true);
        xhr.send(formData);
        new GraphModal().open('success-review');
        thisForm.reset();
    }
    ...
    const validation2 = new JustValidate('#review__form', {
        errorFieldCssClass: 'is-invalid'
    });

    validation2
        .addField('#review__name', [{
            rule: 'required',
            errorMessage: ' '
        }, ])
        .addField('#review__text', [{
            rule: 'required',
            errorMessage: ' '
        }, ])
        .addField('#review__ball', [{
            rule: 'required',
            errorMessage: ' '
        }, ])

    .onSuccess((event) => {
        submitHandler2(document.querySelector('#review__form'));
    });

    let stars = document.querySelectorAll('.raiting__item');

    document.querySelector('.review__button').addEventListener('click', (e) => {
        if (document.getElementById('review__ball').value < 1) {
            for (let item of stars) {
                item.classList.add('raiting__item--error');
            }
        } else if (document.getElementById('review__name' !== '' && document.getElementById('review__text' !== ''))) {
            e.preventDefault();
            for (let item of stars) {
                item.classList.add('raiting__item--error');
            }
        } else if (document.getElementById('review__name').value == '' || document.getElementById('review__text').value == '' || document.getElementById('review__ball').value == 0) {
            document.querySelector('.review__button').classList.add('review__button--disabled');
        }

    });

    document.querySelector('.raiting').addEventListener('click', () => {
        if (document.getElementById('review__ball').value >= 1) {
            for (let item of stars) {
                item.classList.remove('raiting__item--error');
            }
        }

        if (document.getElementById('review__name').value == '' || document.getElementById('review__text').value == '' || document.getElementById('review__ball').value == 0) {
            document.querySelector('.review__button').classList.add('review__button--disabled');
        }

        if (document.getElementById('review__name').value !== '' && document.getElementById('review__text').value !== '') {
            document.querySelector('.review__button').classList.remove('review__button--disabled');
        }
    });

    document.querySelector('#review__name').addEventListener('input', () => {
        if (document.getElementById('review__name').value == '' || document.getElementById('review__text').value == '' || document.getElementById('review__ball').value == 0) {
            document.querySelector('.review__button').classList.add('review__button--disabled');
        }
        if (document.getElementById('review__ball').value !== 0 && document.getElementById('review__text').value !== '') {
            document.querySelector('.review__button').classList.remove('review__button--disabled');
        }
    });

    document.querySelector('#review__text').addEventListener('input', () => {
        if (document.getElementById('review__name').value == '' || document.getElementById('review__text').value == '' || document.getElementById('review__ball').value == 0) {
            document.querySelector('.review__button').classList.add('review__button--disabled');
        }
        if (document.getElementById('review__ball').value !== 0 && document.getElementById('review__name').value !== '') {
            document.querySelector('.review__button').classList.remove('review__button--disabled');
        }
    });
    ...
    let raitingItemsList = document.querySelectorAll('.raiting__item');
    raitingItemsArray = Array.prototype.slice.call(raitingItemsList);
    let itemValue;

    for (let item of raitingItemsList) {
        item.onclick = () => {
            itemValue = item.dataset.itemValue;
            item.parentNode.dataset.totalValue = itemValue;
            document.getElementById('review__ball').value = document.querySelector('.raiting').dataset.totalValue;
        }
    }
